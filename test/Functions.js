...
describe('Functions', () => {
	describe('Example 1', () => {
		...
		it('demonstrates a write function with arguments', async () => {
			await contract.setName('My Example');
			expect(await contract.getName()).to.equal('My Example');
		});
	...
	describe('Example 3', () => {
		it('calls a function from outside the contract', async () => {
			...
			contract.increment();
			contract.increment();
			expect(await contract.count()).to.equal(3);
		});
	});
	...
	describe('Example 4', () => {
		...
		it('can call a public function from outside the contract', async () => {
		...
		it.skip('cannot call an internal function outside the contract', async () => {
	...
	describe.only('Example 6', () => {
		...
		it.only('only lets the owner call the function with a custom modifier', async () => {
			await contract.connect(owner).setName('New Name');
			expect(await contract.name()).to.equal('New Name');
			expect(contract.connect(account1).setName('My Name')).to.be
				.reverted;
		});
		...
		it('demonstrates multiple modifiers', async () => {
			...
			expect(await contract.name()).to.equal('New name');
			expect(await contract.connect(owner).setName1('New name')).to.be
				.reverted;
		});
	});
